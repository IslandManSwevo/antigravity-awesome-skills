...
### Framework-Specific Caching & Container Glitches

**Symptoms**: Next.js/Turbopack throws false-positive CSS parsing errors, or containers fail to start with logging driver errors after resuming work.
**Root causes**: Stale `.next` or `.turbo` cache, orphaned container network states.
**Solutions**:

1. Stop all dev servers (e.g., kill processes matching "next dev" or "nest start").
2. Delete framework caches (`rm -rf .next .turbo`).
3. Fully recreate containers (`docker-compose down && docker-compose up -d`).
4. Regenerate database clients (e.g., Prisma) to sync with fresh containers.
